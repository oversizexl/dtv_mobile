...
仓库内置 GitHub Actions 发布流程：推送 `v*` tag 时会自动构建 APK、生成 GitHub Release，并上传 APK 与 `SHA256SUMS.txt` 校验文件；同时也会在 Actions 运行页的 Artifacts 区域提供 APK 下载。Release 正文会按 semver 取上一个版本 tag，收集其中的 `feat` / `fix` 等功能性 commit 生成 `### 主要更新内容：` 列表，并自动过滤 `docs` / `ci` / `chore` 等维护类提交；重复发布同一 tag 时会覆盖 Release 说明并清理旧 APK，避免 Assets 堆积。tag 也会同步写入 APK 的 `versionName` / `versionCode`。
